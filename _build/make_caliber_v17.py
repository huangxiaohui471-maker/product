# -*- coding: utf-8 -*-
"""生成《全球选品平台_字段口径表_v1.7.xlsx》

v1.7 的改动（对应「法规属性入库 + Hero 情报卡」这一轮，线上 v10）：
  - 字段 39 → 42：商品主数据新增「SKU 数」，可行性新增「备案/许可号」「法规合规声明」
    （三者都来自 TikHub 商品详情，只照录平台原值、不做推测）
  - 待定项 7 结案：TikHub 带回的法规属性与 SKU 结构已入云表
  - 新增增速展示口径：列表与 Hero 一律用中位数，KPI 用截尾均值（去掉最极端 10%）
    —— 原因是真实数据里存在低基数新品（单条环比 +57725%），算术平均会被拉爆
  - 页面新增 Hero 情报卡（把图表翻译成「今天该先看什么」），不改任何洞察规则的判定逻辑
...
BUILD = BASE + "/_build"
OUT = BASE + "/全球选品平台_字段口径表_v1.7.xlsx"
...
SRC_OWNER = {
    ...
    "评价数": "同上",
    "SKU 数": "TikHub 商品详情 sku_count（v1.7 新增）；罗盘 / 蝉妈妈 / FastMoss 商品榜均不提供",
    "备案/许可号": "TikHub 商品详情 legal[]（v1.7 新增）：印尼 BPOM 备案号 58 条、菲律宾 FDA 注册号 15 条；平台没写就留空",
    "法规合规声明": "TikHub 商品详情 legal[]（v1.7 新增）：美国 CA Prop 65 致癌物 / 生殖毒性 14 条，全部为「否」；照录不推测",
...
MAPPING = [
    ...
    ("SKU 数", "—", "—", "sku_count（TikHub 单品接口）",
     "v1.7 新增。商品规格数（容量/色号/组合装），衡量这个品的 SKU 结构复杂度；罗盘与蝉妈妈商品榜都不给"),
    ("备案/许可号", "—", "—", "legal[] 里 name 含 Nomor Ijin Edar / FDA registration 的项",
     "v1.7 新增。目标市场官方备案号原文，如「印尼 BPOM 备案号：NA18260500350」；平台没写就留空，不做推测"),
    ("法规合规声明", "—", "—", "legal[] 里 name 含 CA prop 65 的项",
     "v1.7 新增。法规声明原文，如「美国 CA Prop 65 致癌物：否」；No/Yes 已本地化成 否/是"),
...
TK = [
    ...
    ("法规属性", "有 %d 条，已入云表" % sum(1 for v in DET_OK if v.get("legal")),
     "product_properties 里的法规项，v1.7 起写入云表三个字段：备案/许可号 %d 条（印尼 BPOM %d + 菲律宾 FDA %d）、"
     "法规合规声明 %d 条（美国 CA Prop 65，全部为「否」）、SKU 数 %d 条。"
     "原则：只照录平台原值，平台没写的留空，不用「无」冒充。"
     % (sum(1 for v in DET_OK if any('Ijin Edar' in (x.get('name') or '') or 'FDA' in (x.get('name') or '') for x in (v.get('legal') or []))),
        sum(1 for v in DET_OK if any('Ijin Edar' in (x.get('name') or '') for x in (v.get('legal') or []))),
        sum(1 for v in DET_OK if any('FDA' in (x.get('name') or '') for x in (v.get('legal') or []))),
        sum(1 for v in DET_OK if any('prop 65' in (x.get('name') or '') for x in (v.get('legal') or []))),
        sum(1 for v in DET_OK if isinstance(v.get('sku_count'), int) and v.get('sku_count') > 0))),
]
if CAT:
    s = CAT.get("stat") or {}
...
ws9.append(["项目", "内容"])
NOTES = [
    ("版本", "v1.7 —— 与已上线「全球选品平台」（线上 v10）对齐；法规属性与 SKU 结构入云表（字段 39 → 42），"
            "页面新增 Hero 情报卡，增速展示口径改为中位数 / 截尾均值"),
    ("v1.7 改了什么",
     "① 云表加 3 个字段：「SKU 数」（商品主数据）、「备案/许可号」「法规合规声明」（可行性），全部来自 TikHub 商品详情，"
     "241 条已回填（备案号 73 / 法规声明 14 / SKU 数 241）；"
     "② 页面首屏新增 Hero 情报卡：一句话讲清「哪个国家的哪个子类在集体起量」+ 三个核心数（上升中 / 加速赛道 / 已备案）；"
     "③ 增速展示口径修正：洞察列表与 Hero 用中位数、看板 KPI 用截尾均值（去掉最极端 10%）；"
     "④ 视觉升级：分层阴影 / 圆角节奏 / 卡片微交互 / 进入动效（尊重系统「减少动态效果」设置）"),
    ("v1.7 为什么改口径",
     "真实数据里存在「低基数新品」——印尼一条唇妆的环比是 +57725%，一条品就能把整格均值拉到 +8273%。"
     "均值在这里既不可信也不好读：中位数 0% 又太保守（一半商品持平）。所以列表与 Hero 用中位数（代表常态水位），"
     "看板 KPI 用截尾均值（+18.1%，代表整体水温）。这只改「显示什么数」，不改任何洞察规则的判定逻辑"),
    ("v1.6 改了什么",
    ...
    ("字段数", "选品库 %d 个（v1.7：39 + SKU 数 / 备案许可号 / 法规合规声明）；另建「自有店铺口碑库」21 个字段" % len(FIELDS)),
    ...
    ("待定项 7（已于 v1.7 结案）", "TikHub 带回的法规属性与 SKU 结构已入云表："
                          "「备案/许可号」+「法规合规声明」+「SKU 数」三个字段，241 条已回填。"
                          "后续 P3「法规备案 + 打样」期次可直接用这三个字段做目标市场的合规门槛筛选"),
