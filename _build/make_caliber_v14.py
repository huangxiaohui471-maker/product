...
OUT = BASE + "/全球选品平台_字段口径表_v1.4.xlsx"
...
MAPPING = [
    ...
    ("好评关键词", "罗盘·用户原声（本店）", "—（评价后台）", "—（评价文本，需从商品评价页归纳）", "本店已由「用户原声」自动取到；第三方商品仍需登录评价后台"),
    ("差评关键词", "罗盘·用户原声（本店）", "—（评价后台）", "—（评价文本，需从商品评价页归纳）", "改良机会规则的前提：填了就会自动进「改良机会」；本店已自动取到"),
    ("评价来源", "抖音评价（本店已接通）", "—", "—", "抖音评价 / 天猫评价 / 小红书 / TikTok / 其他；天猫 / 小红书 / TikTok 三渠道浏览器仍未登录"),
...
# ================= Sheet 5b: 自有店铺口碑库（v1.4 新增） =================
ws5b = wb.create_sheet("自有店铺口碑库")
ws5b.append(["字段", "类型", "取数来源", "口径说明"])
REVIEW_FIELDS = [
...
fill_rows(ws5b, REVIEW_FIELDS)
style_sheet(ws5b, [16, 10, 34, 64], 4)
for r in range(2, len(REVIEW_FIELDS) + 2):
    ws5b.row_dimensions[r].height = 30

# ================= Sheet 6: 版本说明 =================
ws6 = wb.create_sheet("版本说明")
ws6.append(["项目", "内容"])
NOTES = [
    ("版本", "v1.4 —— 与已上线「全球选品平台」（线上 v7）对齐；新增第二张表「自有店铺口碑库」与页面第 5 个视图「口碑诊断」"),
    ...
    ("字段数", "选品库 35 个不变；另建「自有店铺口碑库」21 个字段（独立成表，不混入选品库——评价只有本店后台看得到）"),
    ...
    ("待定项 5（评价通道）", "抖音后台已走通（罗盘·体验·用户原声，只覆盖本店商品）；天猫 / 淘宝 / 小红书 / TikTok 在 SageSurf 与 webbridge 两台浏览器上都未登录，需登录后照 _build/wb_douyin_usersound.py 的写法各接一条取数脚本"),
