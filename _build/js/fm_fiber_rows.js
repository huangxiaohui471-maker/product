(function () {
  ...
  if (!fk) return JSON.stringify({ ok: false, err: 'no fiber' });
  ...
  if (!found) return JSON.stringify({ ok: false, err: 'no data array' });
  var keys = ['product_id', 'title', 'region', 'currency', 'real_price', 'commission_rate',
  ...
  var rows = found.map(function (o) {
    var r = {};
    keys.forEach(function (k) { r[k] = (o[k] === undefined ? null : o[k]); });
    r.shop_name = (o.shop_info && o.shop_info.name) || '';
    return r;
  });
  return JSON.stringify({ ok: true, len: rows.length, rows: rows });
})()
