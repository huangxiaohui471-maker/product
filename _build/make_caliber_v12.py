...
OUT = BASE + "/全球选品平台_字段口径表_v1.2.xlsx"
...
RULES = [
    ("窗口期", "环比增速 ≥ 60% 且 关联达人数 ≤ 15 且 销售额 ≥ 100 万",
     "环比增速、关联达人数、销售额", "全部 3 个", "无", "可自动",
     "三个条件全部来自平台数据，导入即可自动出结果。中国、日本、欧美、东南亚可算；韩国因缺销量销售额，一条都参与不了。"),
    ("改良机会", "退货率 ≥ 10% 且 销量 ≥ 3 万",
     "退货率、销量", "销量；退货率部分可导", "无（差评关键词需人工）", "基本可自动",
     "销量随时可导；退货率只有罗盘 / 蝉妈妈部分类目提供，FastMoss 不提供，海外市场这条规则命中率会偏低。差评关键词要人工归纳才有改良方向。"),
    ...
    ("风险提醒", "退货率 ≥ 12% 或 环比增速 < 0",
     "退货率、环比增速", "环比增速；退货率部分可导", "无", "基本可自动",
     "同样受退货率可得性限制；环比增速为负是强信号，优先看这条。"),
]
...
MAPPING = [
    ...
    ("退货率", "退货率（部分类目）", "退货率 / 退款率", "—（不提供）", "海外市场这条规则命中率低"),
...
ws6.append(["项目", "内容"])
NOTES = [
    ("版本", "v1.2 —— 与已上线「全球选品平台」的数据源设计完全对齐"),
    ...
    ("核心结论 3", "33 个字段里平台能自动供的只有 %d 个，另有 %d 个部分可导、%d 个任何源都给不了。5 条洞察规则里有 2 条完全依赖人工字段，导入数据不会自动出结果" % (auto, half, man),
     ),
    ("核心结论 4", "「数据来源」字段此前靠表格里恰好有来源列才会被填，实际几乎全空；现改为导入第 1 步先选源，由源决定市场口径与可得字段，并自动给每条记录打标"),
    ("字段数", "33 个（v1.1 的 32 个 + 决策层新增「数据标记」，用于区分示例数据与真实数据）"),
