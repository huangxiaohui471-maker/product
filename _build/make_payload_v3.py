#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""records_v3.json -> batch_add_database_records 载荷（每批 <=100 条）。"""
import json
import os
...
recs = json.load(open(os.path.join(D, 'records_v3.json'), encoding='utf-8'))['records']
flat = []
for r in recs:
    p = {}
    for k, v in r.items():
...
for i, b in enumerate(batches, 1):
    p = os.path.join(D, 'payload_v3_%d.json' % i)
